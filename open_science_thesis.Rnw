...
\documentclass[color = true, autolayout = false, DIV=15]{ost/subdocuments/open_science_thesis} 

%% Available class options [defaults]
% - color true/[false]: Changes between colored output and a black and white theme (needs some work to setup all the headers with their right colors)
...
% - twosidelayout true/[false]: Switch between two and one side page layout. (remove?) 
