...
\documentclass[oneside, color=true, colormode=rgb, colortheme=red, fancyhf=true, toctosection=chapter, titlepagestyle=image ,BCOR=5mm]{ost/subdocuments/open_science_thesis} 

%% Available class options [defaults]

% titlepagestyle = [simple, image, vertical]
...
% colormode=[rgb, cmyk]
% fancytocs=true,
% toctosection=[part, chapter, false]
% gitinfo=false, 
% sectionnumbers=true,
% autolayout=true,
...
% fancytocs=true/false get a nice layout for your table of contents or use default
% parttocs=true, table of content partials on part page

% - sectionnumbers [true]/false: Switch between numbered and numberless sections  (needs some work)

% - color true/[false]: Changes between colored output and a black and white theme (needs some 
%   work to setup all the headers with their right colors)
...
<<glossary_definitions, child='usr/subdocuments/options/open_science_glossary.Rnw', eval=TRUE>>=
@
...
<<chunk_global_r_options, child='usr/subdocuments/options/ost_global_r_options.Rnw', eval=TRUE>>=
@ 
...
\begin{ostContentTables} 
  \tableofcontents
  \listoffigures 
  \listoftables 
\end{ostContentTables}
...
% <<body_mainmatter_chapter_one, child='usr/subdocuments/chapters/ost_glossaries_section.Rnw', eval=TRUE>>=
% @
